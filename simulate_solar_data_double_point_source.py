...
def makeimage(ra,dec,freq,imagename='sim_onepoint_true.im'):  
	...
	cell=0.5
	shiftx=10
	flux=2./(2*np.pi*sigmax*sigmay)*np.exp(-0.5*((X-shape//2-shiftx)**2/sigmax**2+(Y-shape//2)**2/sigmay**2))
	
	sigmax=2
	sigmay=2
	shiftx=20
	flux=flux+4./(2*np.pi*sigmax*sigmay)*np.exp(-0.5*((X-shape//2-100-shiftx)**2/sigmax**2+(Y-shape//2-100)**2/sigmay**2))
...
generate_ms(config_file,solar_ra,solar_dec,reftime,msname='vla_c_simulated_double_point_shifted.ms')
