...
def makeimage(ra,dec,freq,imagename='sim_onepoint_true.im'):  
	...
	sigmax=4
	sigmay=4
	flux=2./(2*np.pi*sigmax*sigmay)*np.exp(-0.5*((X-shape//2)**2/sigmax**2+(Y-shape//2)**2/sigmay**2))
...
def generate_ms(config_file,source_ra,source_dec,reftime,integration_time=60,msname='fasr.ms',duration=None):
	...
	antenna_params=np.genfromtxt(config_file,usecols=(0,1,2,3))[0:80]
	ant_names=np.genfromtxt(config_file,usecols=(4))[0:80]
	x=antenna_params[:,0]
	y=antenna_params[:,1]
	z=antenna_params[:,2]
...
reftime='2020/02/01/19:00:00'
duration=60

generate_ms(config_file,solar_ra,solar_dec,reftime,msname='fasr_simulated_single_source_no_shift.ms')
